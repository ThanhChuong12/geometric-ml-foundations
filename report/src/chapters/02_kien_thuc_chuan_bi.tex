...
phép đối xứng đối với một họ thuộc tính cho trước tạo thành một nhóm, cấu trúc sẽ được định nghĩa ở mục~\ref{subsec:group-theory}~\cite{bronstein2021geometric}.
\end{definition}
...
đổi $T$ nếu~\cite{bronstein2021geometric}
\begin{equation}
\label{eq:invariance}
...
gian đầu ra sao cho~\cite{cohen2016group}
\begin{equation}
...
\begin{tikzpicture}[>=stealth, font=\small]
  ...
  % Lệnh hỗ trợ vẽ một khối hình chữ L bất đối xứng để nhìn rõ tác động xoay
  \def\drawL#1#2#3{
  ...
  \begin{scope}[xshift=-5cm]
    ...
    % Vị trí đích (Dịch chuyển ngang & dọc, KHÔNG xoay)
  ...
  % ==============================================
  % b) Nhóm xoay tuần hoàn C4 (Rotation)
  % ==============================================
  ...
  % ==============================================
  % c) Nhóm Euclid SE(2) (Quay + Tịnh tiến)
  % ==============================================
  \begin{scope}[xshift=5cm]
    % Vị trí gốc
    \begin{scope}[shift={(0,0)}]
    ...
    % Vị trí đích (Vừa dịch chuyển lên trên, vừa xoay nghiêng -60 độ)
    \begin{scope}[shift={(1.8, 1.1)}, rotate=-60]
      \drawL{teal!10}{teal!50}{dashed}
    \end{scope}
    
    % Mũi tên biểu diễn dời hình cứng
...
\label{def:tac-dong}
Một tác động của nhóm $G$ lên tập $\mathcal{X}$ là một ánh xạ
$G\times\mathcal{X}\to\mathcal{X}$, $(g,x)\mapsto g\cdot x$, thoả mãn~\cite{bronstein2021geometric}:
\begin{equation}
...
$\rho:G\to GL(V)$ tương thích với phép hợp thành~\cite{bronstein2021geometric}:
\begin{equation}
\label{eq:rep-hom}
...
hợp tất cả các điểm có thể đạt tới từ $x$ bằng các phép biến đổi trong $G$~\cite{jegelka2025tutorial}:
\begin{equation}
\label{eq:orbit}
...
\begin{example}
\label{vd:orbit}
Với nhóm $C_{4}$ các phép quay $90^{\circ}$ tác động lên một điểm trong mặt phẳng, quỹ đạo gồm đúng bốn điểm là bốn vị trí quay của điểm đó
...
\begin{tikzpicture}[font=\small]
  ...
  % Đã chỉnh sửa: Đưa chữ "tâm quay" xuống dưới tâm một chút để không đụng ai
  ...
  % Chữ g = 90 độ vẫn giữ nguyên vị trí xích lên an toàn
