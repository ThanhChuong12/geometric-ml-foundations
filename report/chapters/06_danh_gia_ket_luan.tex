...
Lý do phổ biến nhất để dùng tính đối xứng (inductive bias) là giúp tiết kiệm dữ liệu. Điều này rất quan trọng trong hóa học lượng tử hay thiết kế thuốc, vì tính toán mô phỏng tốn rất nhiều thời gian và tiền bạc. Khi có ít dữ liệu, việc đưa thẳng quy luật không gian vào mô hình giúp nó hội tụ nhanh hơn là để nó tự học từ đầu.
...
Dù vậy, các quan sát thực nghiệm trong \cite{jegelka2025tutorial} cho thấy: ngay cả khi có nhiều dữ liệu và máy mạnh, nếu xét cùng một mức chi phí huấn luyện (compute), mô hình equivariant vẫn có sai số thấp hơn mô hình thường. Nghĩa là việc dùng tính đối xứng giúp cải thiện trực tiếp định luật tỷ lệ (scaling laws), làm quá trình học hiệu quả hơn ở mọi quy mô.
...
Ngoài ra, theo \cite{jegelka2025tutorial}, một số tập dữ liệu phân tử (benchmark) nổi tiếng đã bị tác giả âm thầm căn lề (canonicalization) sẵn trước khi công bố. 
...
\textbf{Về ưu điểm:} Lợi ích lớn nhất là học nhanh và cần ít dữ liệu hơn (nhờ inductive bias). Thay vì cần hàng ngàn mẫu ảnh để mô hình biết "ảnh xoay ngược vẫn là ảnh đó", ta cài đặt sẵn quy luật đó vào mạng. Việc này cũng giúp mô hình dự đoán đúng trên các dữ liệu bị biến đổi góc độ chưa từng gặp lúc train (out-of-distribution generalization). 

Bên cạnh đó, nhờ dùng chung trọng số (weight sharing), số tham số cần học ít đi nên giảm được hiện tượng quá khớp (overfitting). Cuối cùng, như đã chỉ ra ở Chương 3, dùng các kỹ thuật như group averaging không làm mất khả năng học được mọi hàm số (universal approximation) của mạng gốc.
...
Thêm vào đó, mô hình equivariant rất khó áp dụng cho các bài toán sinh dữ liệu (generation tasks), vì nhiệm vụ này yêu cầu phá vỡ tính đối xứng (symmetry breaking) để tạo ra cấu trúc mới từ đầu vào đối xứng. Cuối cùng, nếu đoán sai nhóm đối xứng hoặc dùng dữ liệu đã bị căn lề sẵn như đã nói ở trên, hiệu suất mô hình sẽ giảm mạnh.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|L|L|}
        ...
        \hline
        Dự đoán ổn định ngay cả khi dữ liệu đầu vào bị biến đổi (OOD generalization). & Chi phí tính toán cao khi áp dụng group averaging trên các nhóm lớn. \\
        \hline
        Giảm lượng tham số nhờ dùng chung trọng số (weight sharing). & Khó làm các bài toán sinh dữ liệu (vì cần phá vỡ tính đối xứng). \\
        \hline
        Bảo toàn được khả năng xấp xỉ hàm (universal approximation) của mô hình nền. & Hiệu quả giảm mạnh nếu đoán sai tính đối xứng hoặc dùng tập dữ liệu đã căn lề sẵn. \\
        \hline
    \end{tabularx}
...
\subsubsection{Cởi mở hơn với tính đối xứng (Flexible Symmetries)}
...
\subsubsection{Mô hình không phụ thuộc số chiều (Any-dimensional Models)}
...
\subsubsection{Kiểm định tính đối xứng (Symmetry Testing)}
...
\subsubsection{Học trên không gian trọng số (Weight Space Learning)}

Với việc có hàng triệu mô hình mã nguồn mở trên mạng (như Hugging Face), xuất hiện một hướng nghiên cứu là lấy mạng nơ-ron (chính xác là trọng số của nó) làm dữ liệu đầu vào. Trọng số của các lớp ẩn có nhiều phép đối xứng (neural parameter symmetries) \cite{zhao2026symmetry}, ví dụ đổi chỗ các nơ-ron trong mạng MLP thì kết quả dự đoán cuối cùng không hề thay đổi. 
...
Người ta bắt đầu dùng toán học sâu hơn lý thuyết nhóm. Topological Deep Learning \cite{eitan2024topological} dùng phức đơn hình (simplicial complexes) để nhìn được cấu trúc vĩ mô của đồ thị (số lỗ thủng, số liên thông) mà mạng GNN bình thường hay bỏ sót. 
...
Dù dùng phương pháp can thiệp trên dữ liệu (off-the-shelf) hay xây dựng kiến trúc từ đầu (constructed), cách nào cũng có ưu nhược điểm riêng. Data augmentation thì dễ setup và tận dụng được mô hình có sẵn, trong khi các kiến trúc như group convolution thì khó code hơn nhưng lại đảm bảo đúng về mặt toán học.
